...
impl Display for WirehairResultCode {
...
extern "C" {
    ...
    fn wirehair_encoder_create(reuse_codec_opt: *const c_void, message: *const u8, message_size_bytes: u64, block_size_bytes: u32) -> *const c_void;
    fn wirehair_encode(codec: *const c_void, block_id: u64, block: *mut u8, block_size: u32, block_out_bytes: &mut u32) -> WirehairResultCode;
    fn wirehair_decoder_create(reuse_codec_opt: *const c_void, message_size_bytes: u64, block_size_bytes: u32) -> *const c_void;
    fn wirehair_decode(codec: *const c_void, block_id: u64, block: *const u8, block_out_bytes: u32) -> WirehairResultCode;
    fn wirehair_recover(codec: *const c_void, message: *mut u8, message_size_bytes: u64) -> WirehairResultCode;
...
impl Display for WirehairError {
    fn fmt(&self, f: &mut Formatter) -> Result<(), Error> {
        match *self {
            ...
            WirehairError::BadInputSmallN => write!(f, "Too less blocks! Try reducing block size or use a larger message"),
            WirehairError::BadInputLargeN => write!(f, "Too many blocks! Try increasing block_size or use a smaller message"),
            WirehairError::ExtraInsufficient => write!(f, "Not enough extra rows to solve it, possibly corrupted data"),
...
fn parse_wirehair_result(result: WirehairResultCode) -> Result<WirehairResult, WirehairError> {
    match result {
        ...
        _ => Ok(WirehairResult::Other)
    }
}

fn wirehair_init() -> Result<(), WirehairError> {
...
struct WirehairEncoder {
    native_handler: *const c_void
}

fn wirehair_encoder(message: &mut [u8], message_size_bytes: u64, block_size_bytes: u32) -> WirehairEncoder {
    WirehairEncoder {
        native_handler: unsafe {
            wirehair_encoder_create(null::<c_void>(), message.as_ptr(), message_size_bytes, block_size_bytes)
        }
    }
}

impl WirehairEncoder {
    fn encode(&self, block_id: u64, block: &mut [u8], block_size: u32, block_out_bytes: &mut u32) -> Result<WirehairResult, WirehairError> {
        let result = unsafe {
            wirehair_encode(self.native_handler, block_id, block.as_mut_ptr(), block_size, block_out_bytes)
        };
...
struct WirehairDecoder {
    native_handler: *const c_void
}

fn wirehair_decoder(message_size_bytes: u64, block_size_bytes: u32) -> WirehairDecoder {
    WirehairDecoder {
        native_handler: unsafe {
...
impl WirehairDecoder {
    fn decode(&self, block_id: u64, block: &[u8], block_out_size_bytes: u32) -> Result<WirehairResult, WirehairError> {
        let result = unsafe {
            wirehair_decode(self.native_handler, block_id, block.as_ptr(), block_out_size_bytes)
        };

        parse_wirehair_result(result)
    }

    fn recover(&self, message: &mut [u8], message_size_bytes: u64) -> Result<WirehairResult, WirehairError> {
        let result = unsafe {
            wirehair_recover(self.native_handler, message.as_mut_ptr(), message_size_bytes)
        };
...
fn main() {
    ...
    unsafe {
        ...
        let encoder = wirehair_encoder(&mut message, 500, 50);
        let decoder = wirehair_decoder(500, 50);
